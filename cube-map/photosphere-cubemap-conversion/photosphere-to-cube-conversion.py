...
pids = []
args.prefix = ['right','left','front','back','top','bottom']
for i in range(0,6):
    ...
    if args.quality and args.type == "jpg":
        ...
        pimg.save(os.path.join(args.dir, "%s.%s"%(args.prefix[i],args.type)), quality=args.quality)
    else:
        misc.imsave(os.path.join(args.dir, "%s.%s"%(args.prefix[i],args.type)), color_side)
...
if args.onefile:
    ifiles = []
    for i in range(0,6):
        ifiles.append(misc.imread(os.path.join(args.dir, "%s.%s"%(args.prefix[i],args.type))))
    onefile = np.concatenate(ifiles, axis=1)
    misc.imsave(args.onefile, onefile)    
    for i in range(0,6):
        os.unlink(os.path.join(args.dir, "%s.%s"%(args.prefix[i],args.type)))
